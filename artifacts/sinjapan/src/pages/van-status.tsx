            ...
            {currentStep === 3 && (
              <div className="text-center">
                <div className="w-14 h-14 rounded-full bg-foreground flex items-center justify-center mx-auto mb-4">
                  <PackageCheck className="h-7 w-7 text-background" />
                </div>
                <h2 className="text-lg font-bold mb-2">車両を受け取ってください</h2>
                <p className="text-sm text-muted-foreground mb-6">
                  お支払いが完了しました。<br />担当のレンタル会社へ連絡して車両を受け取り、受け取り完了ボタンを押してください。
                </p>
                <div className="flex flex-col sm:flex-row gap-3 justify-center mb-6">
                  <button onClick={() => setLocation(`/van/${applicationId}/pickup`)}
                    className="px-6 py-2.5 border border-border text-sm font-medium rounded-full hover:bg-muted transition-colors flex items-center justify-center gap-2">
                    <MapPin className="h-4 w-4" />受け取り情報を見る
                  </button>
                </div>
                <button onClick={handleConfirmPickup} disabled={submitting}
                  className="w-full max-w-xs mx-auto py-3 bg-foreground text-background text-sm font-medium rounded-full hover:opacity-90 transition-opacity disabled:opacity-40 flex items-center justify-center gap-2">
                  {submitting ? <><Loader2 className="h-4 w-4 animate-spin" />確認中…</> : <><CheckCircle2 className="h-4 w-4" />受け取りました</>}
                </button>
              </div>
            )}
          </div>
