...
【事業者情報】
名称　　: 合同会社SIN JAPAN
所在地　: 〒243-0303 神奈川県愛甲郡愛川町中津7287
TEL　　: 046-212-2325　FAX: 046-212-2326
メール　: info@chat-van.com
制定日　: 2025年4月1日
最終改定: 2026年8月1日
...
(3) 決済処理は当社が契約する決済代行サービス（Square, Inc.）を通じて行われます。クレジットカード情報は当社サーバーには保存されず、同社の安全な環境で管理されます。
(4) 法人のお客様は、当社所定の審査を経て請求書払い（翌月末払い）に変更できます。審査の結果によりご利用いただけない場合があります。
(5) 支払日を過ぎても入金がない場合、年14.6%の遅延損害金が発生します。
(6) 2ヶ月以上の未払いが続いた場合、当社は本契約を解除するとともに、残存する未払い料金を一括請求できるものとします。
(7) 当社は、物価変動・サービス原価の変化等の合理的事由により月額料金を改定できます。改定の場合は1ヶ月前までにユーザーへ通知し、通知後の最初の支払日より改定後の料金を適用します。消費税率の変更は通知なく自動反映されます。
...
所在地　: 〒243-0303 神奈川県愛甲郡愛川町中津7287
TEL　　: 046-212-2325　FAX: 046-212-2326
担当窓口: info@chat-van.com
制定日　: 2025年4月1日
最終改定: 2026年8月1日

合同会社SIN JAPAN（以下「当社」）は、個人情報の保護に関する法律（個人情報保護法）に基づき、ユーザーの個人情報を以下のとおり取り扱います。
...
(5) 支払情報（クレジットカード情報はSquare, Inc.が管理し当社サーバーには保存しません）
...
第3条（外部サービスへの委託）
当社は、以下の外部サービスに個人情報の処理を委託しています。各社の利用規約・プライバシーポリシーが適用されます。
(1) Square, Inc.（決済処理）― クレジットカード情報の安全な管理
(2) Google LLC（クラウドインフラ・ファイルストレージ）― サービス運営基盤
(3) OpenAI, L.L.C.（AIによる審査・チャット）― 審査補助・自動応答（入力内容は学習に使用されません）
...
第7条（開示・訂正・削除・利用停止の請求）
ユーザーは当社に対し、保有する自己の個人情報の開示・訂正・削除・利用停止を請求できます。
お問い合わせ窓口: info@chat-van.com（原則2週間以内に回答）
個人情報保護委員会への苦情申立ても可能です。
個人情報保護委員会: https://www.ppc.go.jp/

第8条（Cookieおよびアクセス解析）
本サービスでは、以下のツールを使用しサービス改善に役立てています。
・セッション管理Cookie（ログイン状態の保持）
ブラウザの設定によりCookieを無効化できますが、ログイン機能等が利用できなくなる場合があります。

第9条（改定）
本方針は、法令変更その他合理的な理由がある場合に改定することがあります。改定時は本サービス上に掲載し、掲載日から効力を生じるものとします。`;

const vehicleTerms = (contract: any) => {
  const v = contract?.vehicle ?? {};
  const company = v.rentalCompany ?? {};
...
契約日　: ${contract?.createdAt ? new Date(contract.createdAt).toLocaleDateString('ja-JP') : new Date().toLocaleDateString('ja-JP')}

【当事者】
貸主（以下「甲」）
　名称　: ${contract?.contractProvider ?? company?.name ?? '協力会社'}
　所在地: ${company?.address ?? '（担当者より別途ご案内）'}
　連絡先: ${company?.email ?? company?.phone ?? 'info@chat-van.com'}
　（合同会社SIN JAPAN提携）
...
【貸渡車両情報】
　メーカー　　: ${v.maker ?? '―'}
　車名・型式　: ${v.model ?? '―'}${v.grade ? `　${v.grade}` : ''}
　年式　　　　: ${v.year ? `${v.year}年式` : '―'}
　車両登録番号: ${v.licensePlate ?? '（受取時に確認）'}
　車台番号　　: ${v.vin ?? '（受取時に確認）'}
　車検満了日　: ${v.inspectionExpiry ?? '―'}
　走行距離　　: ${v.mileage ? `${Number(v.mileage).toLocaleString()} km（引渡時点）` : '（受取時に確認）'}

第1条（貸渡の目的）
甲は乙に対し、上記車両を本契約に基づき貸し渡し、乙はこれを借り受けます。
...
(4) 乙は、安全管理・不正利用防止・事故対応を目的として、車両に搭載されたGPS端末による位置情報の取得・記録に同意します。取得した位置情報は合同会社SIN JAPANの個人情報保護方針に従い管理します。
...
第6条（事故・損害発生時の対応）
(1) 乙は、事故・盗難・損傷が発生した場合は直ちに以下の手順で対応するものとします。
　① 負傷者がいる場合は119番へ通報・救護措置を行う
　② 警察（110番）に届け出る
　③ 甲（${company?.phone ?? company?.email ?? '契約書記載の連絡先'}）へ報告する
　④ 合同会社SIN JAPANサポート（info@chat-van.com）へ報告する
　⑤ 加入している保険会社へ連絡し、事故証明書を取得する
(2) 車両の損傷・事故による修理費用は、乙が加入する保険の適用範囲内で処理します。
(3) 保険適用外の費用・免責額・修理費超過分は乙が負担します。
(4) 乙の故意・重過失による損害については、保険の有無にかかわらず乙が全額負担します。
(5) 事故後の報告を怠った場合、乙は保険適用の有無にかかわらず損害全額を負担する場合があります。

第7条（保険）
(1) 乙は、車両利用前に対人・対物・車両保険（任意保険）に加入するものとします。
(2) 軽貨物事業（配送等）に使用する場合は、黒ナンバー（事業用軽自動車）登録が必要です。黒ナンバー取得手続きについては、当社担当者（info@chat-van.com）へご相談ください。
(3) 黒ナンバー車両の場合は事業用自動車保険（緑ナンバー保険）に加入するものとします。
(4) 保険未加入の場合、甲は乙への車両引き渡しを拒否できます。

第8条（車両の返却）
(1) 乙は、契約終了時に甲の指定する場所へ、貸渡時と同等の状態で車両を返却するものとします。
(2) 車両引き渡し時に甲乙双方が車両の外装・内装・走行距離等の状態を確認し、記録（写真を含む）を作成します。返却時の損傷判定はこの引き渡し時の記録を基準とします。
(3) 通常損耗（走行距離に応じた自然劣化）を超える損傷は乙の費用負担とします。
(4) 所定の日時までに返却されない場合、乙は1日あたり月額料金の1/30相当額を延滞料として支払うものとします。
